...
def community_matrix():
    ...
    while (i < size):
        ...
        while (j < size):
            prompt = str(
                input(f"Does node {i} connect to node {j}? (y/n): \n"))
            if prompt == 'y':
                prompt2 = str(
                    input(f"Is this relationship positive? (y/n): \n"))
                match prompt2.lower():
...
def modularity():
    ...
    while (i < groups):
        ...
        input_node = int(
            input("Input the total number of nodes in this community: \n"))
        nodes.append(input_node)

        input_degree = int(
            input("Input the total degrees of all the edges within this community: \n"))
        degrees.append(input_degree)

        modularity.append((nodes[i] / edges) - (degrees[i] / (2 * edges))**2)

        print(
            f"Nodes: {nodes[i]}, degrees: {degrees[i]}, modularity: {modularity[i]}.\n")
    ...
    for i in range(nodes):
        for j in range(nodes):
            if array[i][j]:
                ...
                print(
                    f"array at {i}, {j}: {array[i][j]} || Modularity at: {modularity}")

    modularity = modularity / (2 * edges)

    print(
        f"Total Edges: {edges} || Total Nodes: {nodes} || Total degrees: {degrees}")
...
def shortest_path(matrix: np.ndarray, start: int, end: int):
    nodes = len(matrix)
    ...
    while priority_queue:
        ...
        for neighbor, weight in enumerate(matrix[current_node]):
            if weight == 1.0 and neighbor not in visited:
...
def input_csv_(file_name: csv) -> np.ndarray:
...
def main():
    ...
    match foo:
        ...
        case "mo":
            com_graph = input_csv(
                str(input("Enter the name of the community graph csv file:\n")))
        ...
        case "ss":
            input_ = input_csv(
                str(input("Enter the name of the matrix: \n")))
            s = int(input("enter the position of the starting node:\n"))
            e = int(input("enter the position of the ending node:\n"))
            print(shortest_path(input_, s, e))
        case "s":  # TODO
            k = np.zeros((3, 3))
